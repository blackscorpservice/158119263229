...
function PT_displayAccountBalance(myOutput) {
    var tempBalancePT = checkBalanceOutput(myOutput, "pt")
...
function aioWM_displayAccountBalance(myOutput) {
    ...
    if (document.contains(document.getElementById("aiowm_balance"))) {
        document.getElementById("aiowm_balance").innerHTML = checkBalanceOutput(myOutput, "aiowm");
    }
...
//GSMiniGame
function GSMiniGame_getAccountBalance(provider, user) {
...
//aioSX
function aioSX_getAccountBalance(provider, user) {
    sendData("api/aiosx/api.aspx", "action=AccountBalance&username=x" + user, aioSX_displayAccountBalance);
}
function aioSX_displayAccountBalance(myOutput) {
    var tempBalanceAIOSX = checkBalanceOutput(myOutput, "aiosx")
    if (tempBalanceAIOSX != "Maintenance" && tempBalanceAIOSX != "Error!") {
        totalBalance = parseFloat(totalBalance) + parseFloat(tempBalanceAIOSX);
        document.getElementById("total_balance").innerHTML = parseFloat(totalBalance).formatMoney(2, ',', '.');
    }
    document.getElementById("aiosx_balance").innerHTML = checkBalanceOutput(myOutput, "aiosx");
    if (document.contains(document.getElementById("aiosx_balance"))) {
        document.getElementById("aiosx_balance").innerHTML = checkBalanceOutput(myOutput, "aiosx");
    }
